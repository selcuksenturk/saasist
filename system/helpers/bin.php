...
$update_server = 'https://app.cloudonex.com';
...
function ui()
{
    global $config, $_L, $saas_url_re_write, $file_build, $resourceURL,$clx_language_code;
...
{
    global $workspace_id;
    $d = ORM::for_table('sys_appconfig')->where('setting', $option)->where('workspace_id',$workspace_id)->find_one();
    if ($d) {
        return false;
    }
    else {
...
{
    global $workspace_id;
    $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting', $option)->find_one();
    if ($d) {
        return $d['value'];
    }
    else {
...
{
    global $workspace_id;
    $d = ORM::for_table('crm_customfieldsvalues')->where('fieldid', $fid)->where('workspace_id',$workspace_id)->where('relid', $rid)->find_one();
    return $d['fvalue'];
}
...
{
    global $workspace_id;
    $d = ORM::for_table('sys_pg')->where('status', 'Active')->where('workspace_id',$workspace_id)->count();
    return $d;
}
...
function db_find_array($table, $columns = array() , $order_by = '')
{
    ...
    $ret = $d->where('workspace_id',$workspace_id)->find_array();
    return $ret;
}
...
{
    global $workspace_id;
    $d = ORM::for_table($table)->where('workspace_id',$workspace_id)->find_one($id);
    if ($d) {
        return $d;
    }
    else {
...
{
    global $workspace_id;
    $d = ORM::for_table('sys_permissions')->where('shortname', $shortname)->where('workspace_id',$workspace_id)->find_one();
    if ($d) {
        return false;
    }
    else {
...
{
    global $workspace_id;
    $d = ORM::for_table('sys_users')->where('workspace_id',$workspace_id)->find_one($aid);
    if ($d) {
        return $d->fullname;
    }
    else {
...
{
    global $workspace_id;
    $d = ORM::for_table('sys_companies')->where('workspace_id',$workspace_id)->find_one($company_id);
    if ($d) {
...
{
    global $workspace_id;
    $d = ORM::for_table('sys_users')->select('img')->where('workspace_id',$workspace_id)->find_one($admin_id);
    if ($d) {
        if ($d->img == 'gravatar') {
...
{
    global $workspace_id;
    $check = EmailTemplate::where('tplname',$template_name)->where('workspace_id',$workspace_id)->first();
    ...
    if($check){
        ...
        $email_template->workspace_id = $workspace_id;
...
{
    global $workspace_id;
    $check = AppConfig::where('setting',$setting)
        ->where('workspace_id',$workspace_id)
        ->first();
...
{
    global $workspace_id;
    $check = AppConfig::where('setting',$setting)->where('workspace_id',$workspace_id)->first();
...
function isValidApiKey($key)
{
    ...
    global $workspace_id;
    $apiKey = ApiKey::where('apikey',$key)->where('workspace_id',$workspace_id)->first();
...
function categoryCalculateTotalByName($name,$type){
    ...
    global $workspace_id;
    $t = Transaction::where('type',$type)->where('category',$name)->where('workspace_id',$workspace_id)->sum('amount');
...
function setSharedPreferences($relation_type,$relation_id,$key,$value)
{
    ...
    if($pref)
    {
        ...
        $pref->workspace_id = $workspace_id;
...
function getTinyMceLocale($language_code)
{
    switch ($language_code)
    {
        case 'en':
...
function getConfig($workspace_id)
{
	$config = [];
	$result = ORM::for_table('sys_appconfig')
	             ->where('workspace_id',$workspace_id)
