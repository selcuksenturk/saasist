...
//        $d = ORM::for_table('sys_accounts')->order_by_desc('balance')->limit(5)->find_many();
        ...
        $tbal = ORM::for_table('sys_accounts')->where('workspace_id',$workspace_id)->sum('balance');
     ...
     //   $ui->assign('d',$d);
        ...
        $home_currency = Currency::where('iso_code',$config['home_currency'])
     ...
     //   $net_worth = ORM::for_table('sys_accounts')->sum('balance');
	    ...
	    $net_worth = Balance::where('workspace_id',$user->workspace_id);

	    if($home_currency)
	    {
	    	$net_worth->where('currency_id',$home_currency->id);
	    }

	    $net_worth = $net_worth->sum('balance');
        ...
        if (has_access($user->roleid, 'transactions', 'all_data')) {
            ...
            $mi = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Income')->where_gte('date',$first_day_month)->where_lte('date',$mdate)->sum('cr');
            if($mi == ''){
                $mi = '0.00';
            }
            $ui->assign('mi',$mi);
            $me = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Expense')->where_gte('date',$first_day_month)->where_lte('date',$mdate)->sum('dr');
            if($me == ''){
                $me = '0.00';
            }
            $ui->assign('me',$me);
            $m = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Income')->where('date',$mdate)->sum('cr');
            if($m == ''){
                $m = '0.00';
            }
            $ui->assign('ti',$m);
            $m = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Expense')->where('date',$mdate)->sum('dr');
            if($m == ''){
                $m = '0.00';
            }
            $ui->assign('te',$m);
            ...
            $customers_total = ORM::for_table('crm_accounts')->where('workspace_id',$user->workspace_id)->count();
            ...
            $companies_total = ORM::for_table('sys_companies')->where('workspace_id',$user->workspace_id)->count();
            ...
            $leads_total = ORM::for_table('crm_leads')->where('workspace_id',$user->workspace_id)->count();
 ...
 ';
            ...
            $invoices = ORM::for_table('sys_invoices')->where('workspace_id',$user->workspace_id)->limit(5)->order_by_desc('id')->find_many();
            $ui->assign('invoices',$invoices);

            $d = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Expense')->limit(5)->order_by_desc('id')->find_many();
            $ui->assign('exp',$d);

            $d = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Income')->limit(5)->order_by_desc('id')->find_many();
            $ui->assign('inc',$d);
            ...
            $total_income = Transaction::where('type','Income')->where('workspace_id',$user->workspace_id)->sum('amount');
            $total_expense = Transaction::where('type','Expense')->where('workspace_id',$user->workspace_id)->sum('amount');
            $total_invoice_paid = Invoice::where('status','Paid')->where('workspace_id',$user->workspace_id)->sum('total');


            $expense_cats = TransactionCategory::where('type','Expense')->where('workspace_id',$user->workspace_id)->orderBy('total_amount', 'desc')->take(10)->get();



            $item_sold = Item::select('name','sold_count','total_amount')->where('workspace_id',$user->workspace_id)->orderBy('sold_count','desc')->take(10)->get();
            ...
            $weekly_total_invoice_amount = Invoice::whereBetween('date', array($weekFromDate, $today_date))->where('workspace_id',$user->workspace_id)->sum('total');
            $weekly_total_invoice_paid = Invoice::whereBetween('date', array($weekFromDate, $today_date))->where('workspace_id',$user->workspace_id)->where('status','Paid')->sum('total');
            $weekly_total_invoice_due = Invoice::whereBetween('date', array($weekFromDate, $today_date))->where('workspace_id',$user->workspace_id)->where('status','Unpaid')->sum('total');


            $monthFromDate = Carbon\Carbon::now()->subDay()->startOfMonth()->toDateString();
            // $monthTillDate = Carbon\Carbon::now()->subDay()->toDateString();
            $monthly_total_invoice_amount = Invoice::whereBetween('date', array($monthFromDate, $today_date))->where('workspace_id',$user->workspace_id)->sum('total');
            $monthly_total_invoice_paid = Invoice::whereBetween('date', array($monthFromDate, $today_date))->where('workspace_id',$user->workspace_id)->where('status','Paid')->sum('total');
            $monthly_total_invoice_due = Invoice::whereBetween('date', array($monthFromDate, $today_date))->where('workspace_id',$user->workspace_id)->where('status','Unpaid')->sum('total');


            $accounts = Account::where('workspace_id',$user->workspace_id)->take(5)->get();
            $currencies = Currency::where('workspace_id',$user->workspace_id)->get();
        }
        else{

            $mi = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Income')->where_gte('date',$first_day_month)->where_lte('date',$mdate)
                ->where('aid',$user->id)
                ->sum('cr');
            if($mi == ''){
                $mi = '0.00';
            ...
            $ui->assign('mi',$mi);
            $me = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Expense')->where_gte('date',$first_day_month)->where_lte('date',$mdate)
                ->where('aid',$user->id)
                ->sum('dr');
            if($me == ''){
                $me = '0.00';
            ...
            $ui->assign('me',$me);
            $m = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Income')->where('date',$mdate)->where('aid',$user->id)->sum('cr');
            if($m == ''){
                $m = '0.00';
            }
            $ui->assign('ti',$m);
            $m = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('type','Expense')->where('date',$mdate)->where('aid',$user->id)->sum('dr');
            if($m == ''){
                $m = '0.00';
            }
            $ui->assign('te',$m);
            ...
            $customers_total = ORM::for_table('crm_accounts')->where('workspace_id',$user->workspace_id)->where('o',$user->id)->count();
            ...
            $companies_total = ORM::for_table('sys_companies')->where('workspace_id',$user->workspace_id)->where('aid',$user->id)->count();
            ...
            $leads_total = ORM::for_table('crm_leads')->where('workspace_id',$user->workspace_id)->where('aid',$user->id)->count();
 ...
 ';
            ...
            $invoices = ORM::for_table('sys_invoices')->where('workspace_id',$user->workspace_id)->where('aid',$user->id)->limit(5)->order_by_desc('id')->find_many();
            $ui->assign('invoices',$invoices);

            $d = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('aid',$user->id)->where('type','Expense')->limit(5)->order_by_desc('id')->find_many();
            $ui->assign('exp',$d);

            $d = ORM::for_table('sys_transactions')->where('workspace_id',$user->workspace_id)->where('aid',$user->id)->where('type','Income')->limit(5)->order_by_desc('id')->find_many();
            $ui->assign('inc',$d);
            ...
            $total_income = Transaction::where('type','Income')->where('workspace_id',$user->workspace_id)->where('aid',$user->id)->sum('amount');
            $total_expense = Transaction::where('type','Expense')->where('workspace_id',$user->workspace_id)->where('aid',$user->id)->sum('amount');
            $total_invoice_paid = Invoice::where('status','Paid')->where('workspace_id',$user->workspace_id)->where('aid',$user->id)->sum('total');
            ...
            $weekly_total_invoice_amount = Invoice::whereBetween('date', array($weekFromDate, $today_date))->where('aid',$user->id)->where('workspace_id',$user->workspace_id)->sum('total');
            $weekly_total_invoice_paid = Invoice::whereBetween('date', array($weekFromDate, $today_date))->where('aid',$user->id)->where('workspace_id',$user->workspace_id)->where('status','Paid')->sum('total');
            ...
            $monthFromDate = Carbon\Carbon::now()->subDay()->startOfMonth()->toDateString();
            // $monthTillDate = Carbon\Carbon::now()->subDay()->toDateString();
            $monthly_total_invoice_amount = Invoice::whereBetween('date', array($monthFromDate, $today_date))->where('aid',$user->id)->where('workspace_id',$user->workspace_id)->sum('total');
            $monthly_total_invoice_paid = Invoice::whereBetween('date', array($monthFromDate, $today_date))->where('aid',$user->id)->where('workspace_id',$user->workspace_id)->where('status','Paid')->sum('total');
            $monthly_total_invoice_due = Invoice::whereBetween('date', array($monthFromDate, $today_date))->where('aid',$user->id)->where('workspace_id',$user->workspace_id)->where('status','Unpaid')->sum('total');
...
var dchart = c3.generate({
 ...
 ');
    ...
    case 'invoice_stats':
        ...
        $paid = ORM::for_table('sys_invoices')->where('status','Paid')->where('workspace_id',$workspace_id)->count();
        $unpaid = ORM::for_table('sys_invoices')->where('status','Unpaid')->where('workspace_id',$workspace_id)->count();
        $partially_paid = ORM::for_table('sys_invoices')->where('status','Partially Paid')->where('workspace_id',$workspace_id)->count();
    ...
    case 'invoice_stats_json':
        ...
        if (has_access($user->roleid, 'transactions', 'all_data')) {
            $paid = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where('status','Paid')->count();
            $unpaid = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where('status','Unpaid')->count();
            $partially_paid = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where('status','Partially Paid')->count();
        } else{
    ...
    case 'update_currency':
        ...
        $currencies = Currency::where('cname', '!=' , $config['home_currency'])->where('workspace_id',$workspace_id)->get();
