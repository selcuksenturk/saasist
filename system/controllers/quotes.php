 ...
 '.
    ...
    case 'edit':
        ...
        if ($d) {
            ...
            $ui->assign('i', $d);
            $items = ORM::for_table('sys_quoteitems')->where('workspace_id',$workspace_id)->where('qid', $id)->order_by_asc('id')->find_many();
...
//find the user
            ...
            $ui->assign('d', $d);
            $ui->assign('_st', $_L['Add Invoice']);
            $c = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->select('id')->select('account')->select('company')->find_many();
            $ui->assign('c', $c);
...
//        $notes = _post('notes');
        ...
        if ($msg == '') {
            ...
            if ($tax != '') {
                $dt = ORM::for_table('sys_tax')->where('workspace_id',$workspace_id)->find_one($tax);
                $taxrate = $dt['rate'];
                $taxname = $dt['name'];
                $taxtype = $dt['type'];
                //
...
//                }
    ...
    case 'list':
        ...
        $total_quote = ORM::for_table('sys_quotes')->where('workspace_id',$workspace_id)->count();

        $ui->assign('total_quote',$total_quote);

        $d = ORM::for_table('sys_quotes')->order_by_desc('id')->where('workspace_id',$workspace_id)->find_array();
...
//                }
            ...
            $d = ORM::for_table('sys_quotes')->where('workspace_id',$workspace_id)->find_one($iid);
            if ($d) {
                ...
                $i = '0';
                $x = ORM::for_table('sys_quoteitems')->where('workspace_id',$workspace_id)->where('qid', $iid)->delete_many();
                foreach ($description as $item) {
                    $samount = $a[$i];
                    /* @since v 2.0 */
                    $sqty = $qty[$i];
...
//            _msglog('s', 'Invoice marked as Dead');
    ...
    case 'mail_invoice_':
        ...
        $d = ORM::for_table('sys_quotes')->where('workspace_id',$workspace_id)->find_one($sid);

        if ($etpl == 'created') {
            $e = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->where('tplname', 'Quote:Quote Created')->find_one();
        } elseif ($etpl == 'reminder') {
            $e = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->where('tplname', 'Invoice:Invoice Payment Reminder')->find_one();
        } elseif ($etpl == 'overdue') {
            $e = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->where('tplname', 'Invoice:Invoice Overdue Notice')->find_one();
        } elseif ($etpl == 'confirm') {
            $e = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->where('tplname', 'Invoice:Invoice Payment Confirmation')->find_one();
        } elseif ($etpl == 'refund') {
            $e = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->where('tplname', 'Invoice:Invoice Refund Confirmation')->find_one();
        } else {
            $d = false;
            $e = false;
        }
...
//        break;
    ...
    case 'convert_invoice':
        ...
        $iid = _post('iid');
        $q = ORM::for_table('sys_quotes')->where('workspace_id',$workspace_id)->find_one($iid);
...
$today = date('Y-m-d');
            ...
            if ($msg == '') {
                ...
                $vtoken = _raid(10);
                $ptoken = _raid(10);
                $d = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->create();
                $d->userid = $q['userid'];
                $d->account = $q['account'];
                $d->date = $idate;
                $d->duedate = $dd;
...
//                $description = $_POST['desc'];
                ...
                $items = ORM::for_table('sys_quoteitems')->where('workspace_id',$workspace_id)->where('qid', $iid)->order_by_asc('id')->find_many();
