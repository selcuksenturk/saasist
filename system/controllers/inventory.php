    ...
    case 'dashboard':
        ...
        foreach ($last_12_months as $month){
          //  echo date('Y-m-d', strtotime($month)).' ';
            ...
            $m['display'][] = $month;
            $m['data'][] = Invoice::where('status','Paid')->where('workspace_id',$workspace_id)->whereBetween('datepaid',array($first_day,$last_day))->sum('total');

        }



        $total_items = Item::where('workspace_id',$workspace_id)->count();
        $total_invoice = Invoice::count();

        $total_invoice_items = InvoiceItem::where('workspace_id',$workspace_id)->sum('qty');
        $total_invoice_amount = ib_money_format(Invoice::where('workspace_id',$workspace_id)->sum('total'),$config);
