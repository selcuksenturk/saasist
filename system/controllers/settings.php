...
switch ($action) {
    case 'expense-categories':
        ...
        $d = ORM::for_table('sys_cats')->where('workspace_id',$workspace_id)->where('type','Expense')->order_by_asc('sorder')->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
...
');
    ...
    case 'expense-categories-post':
        ...
        $c = ORM::for_table('sys_cats')->where('workspace_id',$workspace_id)->where('name',$name)->where('type','Expense')->find_one();
        if($c){
    ...
    case 'income-categories':
        ...
        $d = ORM::for_table('sys_cats')->where('workspace_id',$workspace_id)->where('type','Income')->order_by_asc('sorder')->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
<link rel="stylesheet" type="text/css" href="' . $_theme . '/css/liststyle.css"/>
');
    ...
    case 'income-categories-post':
        ...
        $c = ORM::for_table('sys_cats')->where('workspace_id',$workspace_id)->where('name',$name)->where('type','Income')->find_one();
        if($c){
    ...
    case 'categories-manage':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_cats')->where('workspace_id',$workspace_id)->find_one($id);
        if($d){
            $ui->assign('c',$d);
    ...
    case 'categories-edit-post':
        ...
        if($d){
            ...
            $rd = strtolower($otype);
            $name = _post('name');
            $c = ORM::for_table('sys_cats')->where('workspace_id',$workspace_id)->where('name',$name)->where('type',$otype)->find_one();
            if($c){
    ...
    case 'categories-delete':
        ...
        if($d){
            ...
            ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->raw_query("update sys_transactions set category=:cat where category='$name' AND type='$type'", array('cat' => 'Uncategorized'));
            $d->delete();
            if($type == 'Income'){
...
');
    ...
    case 'payee-edit-post':
        ...
        $id = _post('id');
        $d = ORM::for_table('sys_payee')->where('workspace_id',$workspace_id)->find_one($id);
        if($d){
            $name = _post('name');
            $c = ORM::for_table('sys_payee')->where('workspace_id',$workspace_id)->where('name',$name)->find_one();
            if($c){
            ...
            if($name == ''){
                ...
                $d->save();
                //update payee in transactions
                ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->raw_query("update sys_transactions set payee=:payee where payee='$oname'", array('payee' => $name));
    ...
    case 'payee-post':
        ...
        $c = ORM::for_table('sys_payee')->where('name',$name)->where('workspace_id',$workspace_id)->find_one();
        if($c){
            r2(U."settings/payee",'e',$_L['name_exist_error']);
        }
    ...
    case 'payer':
        ...
        $d = ORM::for_table('sys_payers')->order_by_asc('sorder')->where('workspace_id',$workspace_id)->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
<link rel="stylesheet" type="text/css" href="' . $_theme . '/css/liststyle.css"/>
');
...
');
    ...
    case 'payer-manage':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_payers')->where('workspace_id',$workspace_id)->find_one($id);
        if($d){
            $ui->assign('c',$d);
            view('payer-manage');
        }
    ...
    case 'payer-edit-post':
        ...
        if($d){
            $name = _post('name');
            $c = ORM::for_table('sys_payers')->where('name',$name)->where('workspace_id',$workspace_id)->find_one();
            if($c){
            ...
            if($name == ''){
                ...
                ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->raw_query("update sys_transactions set payer=:payer where payer='$oname'", array('payer' => $name));
    ...
    case 'pmethods':
        ...
        $d = ORM::for_table('sys_pmethods')->order_by_asc('sorder')->where('workspace_id',$workspace_id)->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
<link rel="stylesheet" type="text/css" href="' . $_theme . '/css/liststyle.css"/>
');
...
//');
    ...
    case 'pmethods-manage':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_pmethods')->where('workspace_id',$workspace_id)->find_one($id);
        if($d){
            $ui->assign('c',$d);
    ...
    case 'pmethods-edit-post':
        ...
        if($d){
            $name = _post('name');
            $c = ORM::for_table('sys_pmethods')->where('name',$name)->where('workspace_id',$workspace_id)->find_one();
            if($c){
            ...
            if($name == ''){
                ...
                ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->raw_query("update sys_transactions set pmethod=:pmethod where pmethod='$oname'", array('pmethod' => $name));
    ...
    case 'app':
        ...
        $tblsts = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->raw_query("show table status like 'sys_invoices'")->find_one();
...
minHeight: 150 // pixels
 ...
 ');
    ...
    case 'features':
        ...
        $status_purchase_invoice = Status::where('workspace_id',$workspace_id)->where('type','Purchase Invoice')->get();
...
//');
        $d = ORM::for_table('sys_users')->where('workspace_id',$workspace_id)->find_many();
        $ui->assign('d',$d);
        view('users');
...
//        }
        ...
        $roles = M::factory('Models_Role')->where('workspace_id',$workspace_id)->find_array();
    ...
    case 'users-edit':
      ...
      //  $ui->assign('content_inner',inner_contents($config['c_cache']));
        ...
        $id  = $routes['2'];
        $d = ORM::for_table('sys_users')->where('workspace_id',$workspace_id)->find_one($id);
        if($d){
    ...
    case 'users-post':
        ...
        $r = M::factory('Models_Role')->where('workspace_id',$workspace_id)->find_one($user_type);
...
//check with same name account is exist
        ...
        if($msg == ''){
            ...
            $d->is_email_verified = 1;
            $d->is_phone_verified = 1;
            $d->activation_key = '';
...
//check with same name account is exist
        if($d['username'] != $username){
            $c = ORM::for_table('sys_users')->where('workspace_id',$workspace_id)->where('username',$username)->find_one();
            if($c){
                $msg .= $_L['account_already_exist']. '<br>';
            }
        }
        ...
        if($msg == ''){
            ...
            $d->save();
            r2(U . 'settings/users-edit/'.$id, 's', $_L['Data Updated']);
        }
        else{
    ...
    case 'app-post':
        ...
        if(APP_STAGE == 'Demo'){
            ...
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','CompanyName')->find_one();
            $d->value = $company;
            $d->save();
            ...
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','pdf_font')->find_one();
            $d->value = $pdf_font;
            $d->save();

            $caddress = $_POST['caddress'];
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','caddress')->find_one();
            $d->value = $caddress;
            $d->save();

            $show_quantity_as = $_POST['show_quantity_as'];
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','show_quantity_as')->find_one();
            $d->value = $show_quantity_as;
            $d->save();

            $invoice_terms = $_POST['invoice_terms'];
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','invoice_terms')->find_one();
            $d->value = $invoice_terms;
            $d->save();


            $i_driver = $_POST['i_driver'];
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','i_driver')->find_one();
            $d->value = $i_driver;
            $d->save();
            ...
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','default_landing_page')->find_one();
            $d->value = $default_landing_page;
            $d->save();


            $dashboard = $_POST['dashboard'];
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','dashboard')->find_one();
            $d->value = $dashboard;
            $d->save();
...
//            update_option('contentAnimation',$contentAnimation);
            ...
            if(($iai != '') AND (is_numeric($iai))){
                ...
                $ai = $tblsts['Auto_increment'];
                if($ai < $iai){
                    $set_ai = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->raw_execute("ALTER TABLE sys_invoices auto_increment = $iai");
                }
            }
...
//        update_option('sparkpost_api_key',_post('sparkpost_api_key'));
    ...
    case 'lc-post':
        ...
        $tzone = _post('tzone');
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','timezone')->find_one();
        $d->value = $tzone;
        $d->save();

        $country = _post('country');
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','country')->find_one();
        $d->value = $country;
        $d->save();
...
//        }
        ...
        if($cformat == '1'){
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','dec_point')->find_one();
            $d->value = '.';
            $d->save();
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','thousands_sep')->find_one();
            $d->value = '';
            $d->save();
        }
        elseif($cformat == '2'){
        ...
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','currency_code')->find_one();
...
//        $d->save();
        ...
        $lan = _post('lan');
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','language')->find_one();
        $d->value = $lan;
        $d->save();
    ...
    if($lan == 'ar' || $lan == 'he'){
        ...
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','home_currency')->find_one();
    ...
    if(!$current_home_currency){
        ...
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','currency_decimal_digits')->find_one();
        $d->value = $currency_decimal_digits;
        $d->save();


        $currency_symbol_position = _post('currency_symbol_position');
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','currency_symbol_position')->find_one();
        $d->value = $currency_symbol_position;
        $d->save();

        $thousand_separator_placement = _post('thousand_separator_placement');
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','thousand_separator_placement')->find_one();
        $d->value = $thousand_separator_placement;
        $d->save();

        $calendar_first_day = _post('calendar_first_day');

        update_option('calendar_first_day',$calendar_first_day);
...
//exit;
    ...
    case 'change-password-post':
        ...
        $password = _post('password');
        if($password != ''){
            $d = ORM::for_table('sys_users')->where('workspace_id',$workspace_id)->where('username',$user['username'])->find_one();
            if($d){
                $d_pass = $d['password'];
    ...
    case 'networth_goal':
        ...
        if((is_numeric($goal)) AND $goal != ''){
            $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','networth_goal')->find_one();
            $d->value = $goal;
            $d->save();
    ...
    case 'email-templates':
        $d = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->order_by_desc('id')->find_array();
        $ui->assign('d',$d);
//        $ui->assign('xheader', '
...
//');
    ...
    case 'email-templates-view':
        ...
        if($sid != ''){
            $d = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->find_one($sid);
            if($d){
                $create = false;
            }
            else{
...
<div class="modal-footer">
    ...
    case 'update-email-template':
        ...
        $d = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->find_one($id);
        if(APP_STAGE == 'Demo'){
    ...
    case 'tags':
        ...
        $d = ORM::for_table('sys_tags')->where('workspace_id',$workspace_id)->find_many();
        $ui->assign('d',$d);
...
//        }
    ...
    case 'localisation':
        ...
        $tblsts = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->raw_query("show table status like 'crm_accounts'")->find_one();
    ...
    case 'pg':
        ...
        $d = ORM::for_table('sys_pg')->where('workspace_id',$workspace_id)->order_by_asc('sorder')->find_many();
        $ui->assign('d',$d);
...
//        $ui->assign('xfooter', Asset::js(array('s2/js/select2.min','s2/js/i18n/'.lan())));
    ...
    case 'pg-conf':
        $ui->assign('content_inner',inner_contents($config['c_cache']));
        $pg = $routes['2'];
        $d = ORM::for_table('sys_pg')->where('workspace_id',$workspace_id)->find_one($pg);
        if($d){
...
');
    ...
    case 'edit-tax-post':
        ...
        $tid = _post('tid');
        $d = ORM::for_table('sys_tax')->where('workspace_id',$workspace_id)->find_one($tid);
        if($d){
            $taxname = _post('taxname');
            $taxrate = _post('taxrate');
            $taxrate = Finance::amount_fix($taxrate);
    ...
    case 'consolekey_regen':
        ...
        $d = ORM::for_table('sys_appconfig')->where('workspace_id',$workspace_id)->where('setting','ckey')->find_one();
        $d->value = $nkey;
        $d->save();
    ...
    case 'automation-post':
        ...
        $d = ORM::for_table('sys_schedule')->where('workspace_id',$workspace_id)->where('cname', 'accounting_snapshot')->find_one();
        ...
        $d = ORM::for_table('sys_schedule')->where('workspace_id',$workspace_id)->where('cname', 'recurring_invoice')->find_one();
        ...
        $d = ORM::for_table('sys_schedule')->where('workspace_id',$workspace_id)->where('cname', 'notify')->find_one();
        if ($notify == 'on') {
            $d->val = 'Active';
        } else {
            $d->val = 'Inactive';
        ...
        $d = ORM::for_table('sys_schedule')->where('workspace_id',$workspace_id)->where('cname', 'notifyemail')->find_one();
        $d->val = $notifyemail;
        $d->save();
    ...
    case 'customfields':
        ...
        $cf = ORM::for_table('crm_customfields')->where('workspace_id',$workspace_id)->where('ctype','crm')->order_by_asc('id')->find_many();
    ...
    case 'customfields-ajax-edit':
        ...
        $d = ORM::for_table('crm_customfields')->where('workspace_id',$workspace_id)->find_one($id);
        if($d){
            $ui->assign('d',$d);
...
//    API Support from Version 3
    ...
    case 'api':
        $ui->assign('content_inner',inner_contents($config['c_cache']));
        $d = ORM::for_table('sys_api')->where('workspace_id',$workspace_id)->find_many();
    ...
    case 'api_delete':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_api')->where('workspace_id',$workspace_id)->find_one($id);
        if($d){
    ...
    case 'plugin_force_remove':
        ...
        $d = ORM::for_table('sys_pl')->where('c',$pl)->where('workspace_id',$workspace_id)->find_one();
        if($d){
            $d->delete();
    ...
    case 'activate_license_post':
        ...
        $output = Syscurl::_post('https://activate.cloudonex.com/saas.php',$arr);
...
//        r2(U.'settings/app','s',$_L['Settings Saved Successfully']);
    ...
    case 'add_role':
        ...
        $permissions = M::factory('Models_Permission')->where('workspace_id',$workspace_id)->find_array();
        $roles = M::factory('Models_Role')->where('workspace_id',$workspace_id)->find_array();
    ...
    case 'add_role_post':
        ...
        $check = Role::where('workspace_id',$workspace_id)->where('rname',$rname)->first();
    ...
    case 'edit_role':
        ...
        if($role){
            ...
            $sp = ORM::for_table('sys_staffpermissions')->where('workspace_id',$workspace_id)->where('rid',$id)->find_array();
    ...
    case 'edit_role_post':
        ...
        $role = M::factory('Models_Role')->where('workspace_id',$workspace_id)->find_one($id);
        ...
        if($role){
            ...
            if($msg == ''){
                ...
                $p = ORM::for_table('sys_staffpermissions')->where('workspace_id',$workspace_id)->where('rid',$id)->delete_many();
...
_L[\'are_you_sure\'] = \''.$_L['are_you_sure'].'\';
 ');
        ...
        $currencies = $currency->where('workspace_id',$workspace_id)->find_array();

        if(!$currency->where('workspace_id',$workspace_id)->find_one()){
    ...
    case 'add_currency_post':
        ...
        if($f_type == 'edit'){
            ...
            $cid = _post('cid');
            $currency = M::factory('Models_Currency')->where('workspace_id',$workspace_id)->find_one($cid);
            ...
            $check = M::factory('Models_Currency')->where('workspace_id',$workspace_id)->where('cname',$cname)->find_one();
    ...
    case 'make_base_currency':
        ...
        $c = $currency->where('workspace_id',$workspace_id)->find_one($id);
...
_L[\'are_you_sure\'] = \''.$_L['are_you_sure'].'\';
 ');
        $units = ORM::for_table('sys_units')->where('workspace_id',$workspace_id)->order_by_asc('sorder')->find_array();
    ...
    case 'modal_unit':
        ...
        if($id != ''){
            ...
            $unit = ORM::for_table('sys_units')->where('workspace_id',$workspace_id)->find_one($id);
    ...
    case 'add_unit':
        ...
        if($f_type == 'edit'){
            $d = ORM::for_table('sys_units')->where('workspace_id',$workspace_id)->find_one($uid);
            $d->name = $name;
            $d->type = $type;
            $d->save();
            echo $d->id();
    ...
    case 'clone_email_template':
        ...
        if($d){
            ...
            for ($x = 2; $x <= 30; $x++) {
                $e = ORM::for_table('sys_email_templates')->where('workspace_id',$workspace_id)->where('tplname',$d->tplname.': '.$x)->find_one();
                if($e){
                    continue;
                }
                else{
    ...
    case 'expense-types':
        ...
        $e = ORM::for_table('expense_types')->order_by_asc('sorder')->where('workspace_id',$workspace_id)->find_array();
...
_L[\'are_you_sure\'] = \''.$_L['are_you_sure'].'\';
 ');
    ...
    case 'e_expense_type_edit':
        ...
        $d = ORM::for_table('expense_types')->where('workspace_id',$workspace_id)->find_one($id);
	...
	case 'billing':
		...
		$current_plan = $_L['Trial'];

		$workspace = Workspace::find($workspace_id);

		if(isset($config['current_plan']))
		{
		...
		$super_home_currency = 'USD';

		$super_admin_config_currency = Setting::where('workspace_id',1)
			->where('setting','home_currency')
			->first();

		if($super_admin_config_currency)
		{
			$super_home_currency = $super_admin_config_currency->value;
		}

		// Check user has unpaid invoice


		$active_invoice = false;
		$invoices = [];
		$customer = Contact::where('billingcid',$workspace_id)->first();

		if($customer)
		{
			$invoices = Invoice::where('userid',$customer->id)->get();
			$active_invoice = Invoice::where('userid',$customer->id)
				->where('status','Unpaid')
				->orderBy('id','desc')
				->first();
		}
		...
		view('billing',[
			...
			'plans' => $plans,
			'super_home_currency' => $super_home_currency,
	...
	case 'subscribe_post':
		...
		if($plan_id == '')
		{
			responseWithError($_L['Choose plan']);
		}
		...
		if($plan)
		{
			...
			$customer = Contact::where('billingcid',$workspace_id)
				->first();
			...
			if($customer)
			{
				...
				$customer->workspace_id = 1;
				$customer->billingcid = $workspace_id;
				$customer->account = $user->fullname;
			...
			// check customer has already unpaid invoice

			$invoice = Invoice::forSaaSPlan($customer_id,$plan);
