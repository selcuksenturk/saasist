...
if(isset($config['plan']))
{
    ...
    if($workspace_plan)
    {
        ...
        if(!isset($enabled_modules['customers']))
        {
            permissionDenied();
        }
...
_L[\'New Company\'] = \''.$_L['New Company'].'\';
 ');
        ...
        $currencies = M::factory('Models_Currency')
    ...
    case 'summary':
        ...
        $cid = _post('cid');
        $d = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->find_one($cid);
        if($d){
            ...
            $cf = ORM::for_table('crm_customfields')->where('ctype','crm')->where('workspace_id',$workspace_id)->order_by_asc('id')->find_many();
            $ui->assign('cf',$cf);
    ...
    case 'activity':
        ...
        $d = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->find_one($cid);
        if($d){
            $ac = ORM::for_table('sys_activity')->where('workspace_id',$workspace_id)->where('cid',$cid)->limit(20)->order_by_desc('id')->find_many();
            $ui->assign('ac',$ac);
    ...
    case 'invoices':
        ...
        $d = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->find_one($cid);
        if($d){
$i = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where('userid',$cid)->find_many();

            $total_invoice_amount = Invoice::where('userid',$cid)->where('workspace_id',$workspace_id)->sum('total');
            $total_paid_amount = Invoice::where('userid',$cid)->where('workspace_id',$workspace_id)->paid()->sum('total');
            $total_unpaid_amount = Invoice::where('userid',$cid)->where('workspace_id',$workspace_id)->unpaid()->sum('total');
    ...
    case 'purchases':
        ...
        if($d){
            $i = ORM::for_table('sys_purchases')->where('userid',$cid)->where('workspace_id',$workspace_id)->find_many();

            $total_invoice_amount = Purchase::where('userid',$cid)->where('workspace_id',$workspace_id)->sum('total');
            $total_paid_amount = Purchase::where('userid',$cid)->where('workspace_id',$workspace_id)->paid()->sum('total');
            $total_unpaid_amount = Purchase::where('userid',$cid)->where('workspace_id',$workspace_id)->unpaid()->sum('total');
    ...
    case 'quotes':
        ...
        $d = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->find_one($cid);
        if($d){
            $i = ORM::for_table('sys_quotes')->where('workspace_id',$workspace_id)->where('userid',$cid)->find_many();
            $ui->assign('i',$i);
    ...
    case 'email':
        ...
        if($d){
            $e = ORM::for_table('sys_email_logs')
                ->where('userid',$cid)
                ->order_by_desc('id')->where('workspace_id',$workspace_id)->find_many();
            $ui->assign('d',$d);
            $ui->assign('e',$e);
    ...
    case 'edit':
        ...
        $d = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->find_one($cid);
        if($d){
            $fs = ORM::for_table('crm_customfields')->where('workspace_id',$workspace_id)->where('ctype','crm')->order_by_asc('id')->find_many();
            $ui->assign('fs',$fs);
    ...
    case 'activity-delete':
        ...
        $id = $routes['3'];
        $d = ORM::for_table('sys_activity')->where('workspace_id',$workspace_id)->find_one($id);
        $d->delete();
        $cid = $routes['2'];
    ...
    case 'view':
        ...
        if($d){
            ...
            if($d->type == 'Supplier'){
                ...
                $po_count = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->where('userid',$id)->count();
            ...
            $inv_count = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where('userid',$id)->count();
            ...
            $quote_count = ORM::for_table('sys_quotes')->where('workspace_id',$workspace_id)->where('userid',$id)->count();
...
//        }
        ...
        if($msg == ''){
          ...
          //  $type = _post('type');
            ...
            $fs = ORM::for_table('crm_customfields')->where('workspace_id',$workspace_id)->where('ctype','crm')->order_by_asc('id')->find_array();
            foreach($fs as $f){
                $fvalue = _post('cf'.$f['id']);
                $fc = ORM::for_table('crm_customfieldsvalues')->create();
                $fc->workspace_id = $workspace_id;
                $fc->fieldid = $f['id'];
                $fc->relid = $cid;
                $fc->fvalue = $fvalue;
                $fc->workspace_id = $workspace_id;
...
//            }
                ...
                if($email != ($d['email'])){
                    $f = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->where('email',$email)->find_one();
...
//            }
            ...
            if($msg == ''){
                ...
                $exf = ORM::for_table('crm_customfieldsvalues')->where('workspace_id',$workspace_id)->where('relid',$id)->delete_many();
                $fs = ORM::for_table('crm_customfields')->where('workspace_id',$workspace_id)->order_by_asc('id')->find_many();
                foreach($fs as $f){
                    $fvalue = _post('cf'.$f['id']);
                    $fc = ORM::for_table('crm_customfieldsvalues')->create();
                    $fc->workspace_id = $workspace_id;
...
//            _msglog('s',$result);
    ...
    case 'groups':
        ...
        $gs = ORM::for_table('crm_groups')->order_by_asc('sorder')->where('workspace_id',$workspace_id)->find_array();
 ...
 ');
    ...
    case 'add_group':
        ...
        if($group_name != ''){
            ...
            $c = ORM::for_table('crm_groups')->where('workspace_id',$workspace_id)->where('gname',$group_name)->find_one();
    ...
    case 'find_by_group':
        ...
        if($gid){
            ...
            $g = ORM::for_table('crm_groups')->where('workspace_id',$workspace_id)->find_one($gid);

            if($g){

                $d = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->where('gid',$gid)->order_by_desc('id')->find_array();
...
');
    ...
    case 'group_edit':
        ...
        $d = ORM::for_table('crm_groups')->where('workspace_id',$workspace_id)->find_one($id);
    ...
    case 'group_email':
        ...
        if($gid){
            ...
            $ds = ORM::for_table('crm_accounts')->where('gid',$gid)->where('workspace_id',$workspace_id)->where_not_equal('email','')->select('account')->select('email')->order_by_desc('id')->find_array();
...
// find all companies
        ...
        $companies = Company::orderBy('id','desc')->where('workspace_id',$workspace_id)->get()->toArray();
...
//            $val[''] = $company->;
    ...
    case 'edit_activity_post':
        ...
        $d = ORM::for_table('sys_activity')->where('workspace_id',$workspace_id)->find_one($edit_activity_id);
    ...
    case 'files':
        ...
        $file_ids = ORM::for_table('ib_doc_rel')->where('rtype','contact')->where('workspace_id',$workspace_id)->where('rid',$cid)->find_array();
        ...
        if (!empty($ids)) {
            ...
            $d = ORM::for_table('sys_documents')->where('workspace_id',$workspace_id)->where_in('id', $ids)->find_many();
        ...
        $files = ORM::for_table('sys_documents')->where('workspace_id',$workspace_id)->find_array();
    ...
    case 'assign_file':
        ...
        $check = ORM::for_table('ib_doc_rel')->where('workspace_id',$workspace_id)->where('rtype','contact')->where('rid',$cid)->where('did',$did)->find_one();
    ...
    case 'remove_file':
        ...
        $d = ORM::for_table('ib_doc_rel')->where('workspace_id',$workspace_id)->where('rtype','contact')->where('rid',$cid)->where('did',$did)->find_one();
    ...
    case 'modal_view_company':
        ...
        $company = ORM::for_table('sys_companies')->where('workspace_id',$workspace_id)->find_one($id);
    ...
    case 'company_memo':
        ...
        $d = ORM::for_table('sys_companies')->where('workspace_id',$workspace_id)->find_one($cid);
    ...
    case 'company_update_notes':
        ...
        $d = ORM::for_table('sys_companies')->where('workspace_id',$workspace_id)->find_one($id);
    ...
    case 'company_customers':
        ...
        $customers = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->select('id')->select('account')->select('email')->select('phone')->where('cid',$cid)->find_array();
        ...
        if($d){
          ...
          //  var_dump($invoices);
            ...
            if($customers){
                ...
                $invoices = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where_in('userid',$customers)->find_array();
    ...
    case 'company_quotes':
        ...
        if($d){
            ...
            if($customers){
                ...
                $quotes = ORM::for_table('sys_quotes')->where('workspace_id',$workspace_id)->where_in('userid',$customers)->find_array();
    ...
    case 'company_orders':
        ...
        if($d){
            ...
            if($customers){
                ...
                $orders = ORM::for_table('sys_orders')->where('workspace_id',$workspace_id)->where_in('cid',$customers)->find_array();
...
//        exit;
        ...
        $d->where('workspace_id',$user->workspace_id);
    ...
    case 'add_fund':
        ...
        $customer = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->find_one($cid);
...
//            }
    ...
    case 'log':
        ...
        $logs = ORM::for_table('sys_logs')->where('type','Client')->where('workspace_id',$workspace_id)->where('userid',$cid)->limit(1000)->order_by_desc('id')->find_array();
    ...
    case 'client-password-manager':
        ...
        $passwords = PasswordManager::where('client_id',$cid)->where('workspace_id',$workspace_id)->get();
    ...
    case 'credit_card_info':
        ...
        $contact = Contact::where('workspace_id',$workspace_id)->find($cid);

        //

        $credit_card = CreditCard::where('contact_id',$cid)->where('workspace_id',$workspace_id)->first();
    ...
    case 'save_credit_card':
        ...
        if($contact){
            ...
            if(!$credit_card){
                ...
                $credit_card->workspace_id = $workspace_id;
            }
