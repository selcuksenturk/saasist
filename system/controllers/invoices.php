...
if(isset($config['plan']))
{
    ...
    if($workspace_plan)
    {
        ...
        if(!isset($enabled_modules['invoicing']))
        {
            permissionDenied();
        }
...
.redactor-editor {
 ...
 ' . $extra_jq);
    ...
    case 'edit':
        ...
        $id = $routes['2'];
        $d = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->find_one($id);
        if ($d) {
            ...
            $ui->assign('currencies', $currencies);
            $ui->assign('i', $d);
            $items = ORM::for_table('sys_invoiceitems')->where('workspace_id',$workspace_id)->where('invoiceid', $id)->order_by_asc('id')->find_many();
            ...
            $ui->assign('d', $d);
            $ui->assign('_st', $_L['Add Invoice']);
            $c = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->select('id')->select('account')->select('company')->select('email')->order_by_desc('id')->where_like('type','%Customer%')->find_many();
            $ui->assign('c', $c);
            $t = ORM::for_table('sys_tax')->where('workspace_id',$workspace_id)->find_many();
            $ui->assign('t', $t);
 ...
 ');
    ...
    case 'view':
        ...
        if ($d) {
            ...
            $emls_c = ORM::for_table('sys_email_logs')->where('workspace_id',$workspace_id)->where('iid', $id)->count();
            $emls = ORM::for_table('sys_email_logs')->where('workspace_id',$workspace_id)->where('iid', $id)->order_by_desc('id')->find_many();
            $ui->assign('emls', $emls);
    ...
    aSep: \'' . $config['thousands_sep'] . '\'
            ...
            if($a->cid != '' || $a->cid != 0){
                $company = Company::where('workspace_id',$workspace_id)->find($a->cid);
            }
            else{
                $company = false;
            }
            ...
            if($d->quote_id != '0'){
                $quote = ORM::for_table('sys_quotes')->where('workspace_id',$workspace_id)->find_one($d->quote_id);
            }
...
//                }
    ...
    case 'list':
        ...
        $total_invoice = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id);
...
$(\'[data-toggle="tooltip"]\').tooltip();
 ...
 ');
        ...
        $paid = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id);
        ...
        $paid =  $paid->where('status', 'Paid')->where('workspace_id',$workspace_id)->count();

        $unpaid = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where('status', 'Unpaid');
        ...
        $partially_paid = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where('status', 'Partially Paid');
        ...
        $cancelled = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where('status', 'Cancelled');
        ...
        $invoice_paid_amount = Invoice::where('workspace_id',$workspace_id)->where('status', 'Paid');
        ...
        $invoice_un_paid_amount = Invoice::where('status', 'Unpaid')->where('workspace_id',$workspace_id);
        ...
        $invoice_partially_paid_amount = Invoice::where('status', 'Partially Paid')->where('workspace_id',$workspace_id);
        ...
        $invoice_cancelled_amount = Invoice::where('status', 'Cancelled')->where('workspace_id',$workspace_id);
    ...
    case 'list-recurring':
        $d = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->where_not_equal('r', '0')->order_by_desc('id')->find_many();
        $ui->assign('d', $d);
        $ui->assign('xjq', '
$(".cdelete").click(function (e) {
        e.preventDefault();
...
//            }
            ...
            $d = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->find_one($iid);
            if ($d) {
                $d->userid = $cid;
                $d->account = $u['account'];
                $d->date = $idate;
                ...
                $description = $_POST['desc'];
                $i = '0';
                $inventory_items_adjust = ORM::for_table('sys_invoiceitems')->where('workspace_id',$workspace_id)->where('invoiceid', $iid)->find_array();
                foreach($inventory_items_adjust as $i_adjust) {
                    Inventory::increaseByItemNumber($i_adjust['itemcode'], $i_adjust['qty']);
                }

                $x = ORM::for_table('sys_invoiceitems')->where('workspace_id',$workspace_id)->where('invoiceid', $iid)->delete_many();
...
//                    }
    ...
    case 'markpaid':
        ...
        if ($d) {
            ...
            if($prev_status == 'Cancelled')
            {
                ...
                $items = InvoiceItem::where('invoiceid',$iid)->where('workspace_id',$workspace_id)->get();
            ...
            _msglog('e', $_L['Invoice not found']);
        }
    ...
    case 'markpartiallypaid':
        ...
        if ($d) {
            ...
            $d->save();
            _msglog('s', $_L['Invoice marked as partially paid']);
        }
        else {
            _msglog('e', 'Invoice not found');
        }
    ...
    case 'add-payment':
        ...
        $sid = $routes['2'];
        $d = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->find_one($sid);
        if ($d) {
            $itotal = $d['total'];
            $ic = $d['credit'];
            $np = $itotal - $ic;
...
<input type="hidden" id="payer" name="payer" value="' . $d['userid'] . '">
    ...
    case 'add-payment-post':
        ...
        if($payerid != '0'){
            $payer_find = Contact::where('workspace_id',$workspace_id)->find($payerid);
...
//        }
        ...
        if ($msg == '') {
            ...
            $i = ORM::for_table('sys_invoices')->where('workspace_id',$workspace_id)->find_one($iid);
            if ($i) {
                $pc = $i['credit'];
                $it = $i['total'];
                $dp = $it - $pc;
