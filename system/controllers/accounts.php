...
switch ($action) {
    case 'balances':
        ...
        $home_currency = Currency::where('iso_code', $config['home_currency'])->where('workspace_id',$user->workspace_id)->first();

        //   $net_worth = ORM::for_table('sys_accounts')->sum('balance');

        $net_worth = Balance::where('currency_id', $home_currency->id)->where('workspace_id',$user->workspace_id)->sum('balance');
        ...
        if($all_data)
        {
            $accounts = Account::where('workspace_id',$user->workspace_id)->get();
        } else{
            $accounts = Account::where('owner_id',$user->id)->where('workspace_id',$user->workspace_id)->get();
        }
    ...
    case 'add-post':
        ...
        $d = ORM::for_table('sys_accounts')->where('workspace_id',$user->workspace_id)->where('account', $account)->find_one();
        if ($d) {
        ...
        if ($msg == '') {
            ...
            $d->workspace_id = $user->workspace_id;
            $d->account = $account;
    ...
    case 'list':
        ...
        $d = $d->where('workspace_id',$user->workspace_id)->find_many();
    ...
    case 'edit':
        $id = $routes['2'];
        $d = ORM::for_table('sys_accounts')->where('workspace_id',$user->workspace_id)->find_one($id);
        if ($d) {
            $ui->assign('d', $d);
            view('account-edit');
        }
    ...
    case 'delete':
        ...
        if ($d) {
            ...
            $balances = Balance::where('account_id',$id)->where('workspace_id',$user->workspace_id)->get();
