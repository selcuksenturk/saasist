...
if(isset($config['plan']))
{
    ...
    if($workspace_plan)
    {
        ...
        if(!isset($enabled_modules['purchase']))
        {
            permissionDenied();
        }
...
switch ($action) {
    ...
    case 'add':
        ...
        $c = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->select('id')->select('account')->select('company')->select('email')->order_by_desc('id')->where_like('type','%Supplier%')->find_many();
        $ui->assign('c', $c);
        $t = ORM::for_table('sys_tax')->where('workspace_id',$workspace_id)->find_many();
        $ui->assign('t', $t);
...
.redactor-editor {
 ...
 ' . $extra_jq);
        ...
        $tax_default = ORM::for_table('sys_tax')->where('workspace_id',$workspace_id)->where('is_default',1)->find_one();
    ...
    case 'edit':
        ...
        $id = $routes['2'];
        $d = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->find_one($id);
        if ($d) {
            $currencies = M::factory('Models_Currency')->where('workspace_id',$workspace_id)->find_array();
            $ui->assign('currencies', $currencies);
            $ui->assign('i', $d);
            $items = ORM::for_table('sys_purchaseitems')->where('workspace_id',$workspace_id)->where('invoiceid', $id)->order_by_asc('id')->find_many();
...
.redactor-editor {
 ...
 ');
    ...
    case 'edit-post':
        ...
        $u = ORM::for_table('crm_accounts')->where('workspace_id',$workspace_id)->find_one($cid);
        $msg = '';
        if ($cid == '') {
        ...
        $currency_find = Currency::where('workspace_id',$workspace_id)->find($currency_id);
        if ($currency_find) {
...
//            }
            ...
            $d = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->find_one($iid);
            if ($d) {
                $d->userid = $cid;
                $d->account = $u['account'];
                $d->date = $idate;
...
//                    }
    ...
    case 'list':
        ...
        $total_invoice = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->count();
        $ui->assign('total_invoice', $total_invoice);
        $f = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id);
        if (route(3) != '') {
            $s_f = route(3);
            if ($s_f == 'paid') {
                $f->where('status', 'Paid');
        ...
        $d = $f->order_by_desc('id')->where('workspace_id',$workspace_id)->find_many();
        $paginator['contents'] = '';
...
$(\'[data-toggle="tooltip"]\').tooltip();
 ...
 ');
        $paid = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->where('status', 'Paid')->count();
        $unpaid = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->where('status', 'Unpaid')->count();
        $partially_paid = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->where('status', 'Partially Paid')->count();
        $cancelled = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->where('status', 'Cancelled')->count();
        $arr = array(
            'Paid' => $paid,
            'Unpaid' => $unpaid,
        ...
        $invoice_paid_amount = Purchase::where('status', 'Paid')->where('workspace_id',$workspace_id)->sum('total');
        $invoice_un_paid_amount = Purchase::where('status', 'Unpaid')->where('workspace_id',$workspace_id)->sum('total');
        $invoice_partially_paid_amount = Purchase::where('status', 'Partially Paid')->where('workspace_id',$workspace_id)->sum('credit');
        $invoice_cancelled_amount = Purchase::where('status', 'Cancelled')->where('workspace_id',$workspace_id)->sum('total');
        $cancelled = Purchase::where('status', 'Cancelled')->where('workspace_id',$workspace_id)->count();
    ...
    case 'view':
        ...
        if ($d) {
            ...
            $trs_c = ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->where('purchase_id', $id)->count();
            $trs = ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->where('purchase_id', $id)->order_by_desc('id')->find_many();
            $ui->assign('trs', $trs);
            $ui->assign('trs_c', $trs_c);
            $emls_c = ORM::for_table('sys_email_logs')->where('workspace_id',$workspace_id)->where('purchase_id', $id)->count();
            $emls = ORM::for_table('sys_email_logs')->where('workspace_id',$workspace_id)->where('purchase_id', $id)->order_by_desc('id')->find_many();
            $ui->assign('emls', $emls);
            ...
            $cf = ORM::for_table('crm_customfields')->where('workspace_id',$workspace_id)->where('showinvoice', 'Yes')->order_by_asc('id')->find_many();
            $ui->assign('cf', $cf);
            $ui->assign('xheader', Asset::css(array(
                's2/css/select2.min',
                'dropzone/dropzone',
    ...
    vMax: \'9999999999999999.00\',
            ...
            $access_logs = ORM::for_table('ib_invoice_access_log')->where('workspace_id',$workspace_id)->where('iid', $id)->order_by_desc('id')->find_array();
...
;
    ...
    case 'pdf':
        ...
        if ($d) {
            ...
            $items = ORM::for_table('sys_purchaseitems')->where('workspace_id',$workspace_id)->where('invoiceid', $id)->order_by_asc('id')->find_many();
            $trs_c = ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->where('iid', $id)->count();
            $trs = ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->where('iid', $id)->order_by_desc('id')->find_many();
    ...
    case 'markpaid':
        ...
        if ($d) {
            ...
            _msglog('s', $_L['Invoice marked as paid']);
        }
        else {
    ...
    case 'markunpaid':
        ...
        if ($d) {
            $d->status = 'Unpaid';
            $d->save();
            _msglog('s', $_L['Invoice marked as unpaid']);
        }
        else {
            _msglog('e', 'Invoice not found');
        }
    ...
    case 'markcancelled':
        ...
        if ($d) {
            $d->status = 'Cancelled';
            $d->save();
            _msglog('s', $_L['Invoice marked as cancelled']);
        }
        else {
            _msglog('e', 'Invoice not found');
        }
    ...
    case 'add-payment':
        ...
        $sid = $routes['2'];
        $d = ORM::for_table('sys_purchases')->where('workspace_id',$workspace_id)->find_one($sid);
        if ($d) {
            $itotal = $d['total'];
            $ic = $d['credit'];
            $np = $itotal - $ic;
            ...
            $a = ORM::for_table('sys_accounts')->where('workspace_id',$workspace_id)->find_many();
            foreach($a as $acs) {
            ...
            $pms = ORM::for_table('sys_pmethods')->where('workspace_id',$workspace_id)->order_by_asc('sorder')->find_many();
            foreach($pms as $pm) {
                $pms_opt.= '<option value="' . $pm['name'] . '">' . $pm['name'] . '</option>';
            }

            $cats_opt = '';
            $cats = ORM::for_table('sys_cats')->where('workspace_id',$workspace_id)->where('type', 'Expense')->order_by_asc('sorder')->find_many();
            foreach($cats as $cat) {
...
<input type="hidden" id="payer" name="payer" value="' . $d->userid . '">
    ...
    case 'add-payment-post':
        ...
        $currencies = Currency::where('workspace_id',$workspace_id)->all();
        $tr_currency = '0';
        $tr_currency_symbol = '';
...
//        }
        ...
        if ($msg == '') {
            ...
            $home_currency = Currency::where('workspace_id',$workspace_id)->where('iso_code', $config['home_currency'])->first();
            $account_balance = Balance::where('workspace_id',$workspace_id)->where('account_id', $account_id)->where('currency_id', $home_currency->id)->first();
            if ($account_balance) {
...
<div class="modal-footer">
    ...
    case 'payments':
        ...
        $d = ORM::for_table('sys_transactions')->where('workspace_id',$workspace_id)->where_not_equal('iid', '0')->limit(500)->find_array();
        $ui->assign('d', $d);
        $ui->assign('xheader', $mode_css);
        $ui->assign('xfooter', $mode_js);
        $ui->assign('xjq', '
...
.redactor-editor {
 ...
 ' . $extra_jq);
    ...
    case 'set_stage':
        ...
        $purchase = Purchase::where('workspace_id',$workspace_id)->find($id);
