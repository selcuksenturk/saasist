...
if(isset($config['plan']))
{
    ...
    if($workspace_plan)
    {
        ...
        if(!isset($enabled_modules['calendar']))
        {
            permissionDenied();
        }
...
var ib_date_format_moment = \''.ib_js_date_format($config['df']).'\';
 ');
    ...
    case 'data':
        ...
        $x = M::factory('Models_Calendar')->where('workspace_id',$workspace_id)->where_gte('start',$start)->where_lte('end',$end)->select('title')->select('start')->select('end')->select('description','_tooltip')->select('id','eventid')->select('color');
