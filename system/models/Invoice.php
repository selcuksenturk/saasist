...
//            $pdf_tpl = 'system/lib/invoices/left-logo.php';
	...
	public static function forSaaSPlan($cid,$plan){
		...
		if (isset($currencies[$super_home_currency])) {
			$selected_currency = $currencies[$super_home_currency];
    ...
    public static function files($invoice_id){
        ...
        $file_ids = ORM::for_table('ib_doc_rel')->where('workspace_id',$workspace_id)->where('rtype','invoice')->where('rid',$invoice_id)->find_array();
    ...
    public static function cloneInvoice($id)
    {
        ...
        if ($inv) {
            ...
            $items = ORM::for_table('sys_invoiceitems')->where('workspace_id',$workspace_id)->where('invoiceid', $id)->order_by_asc('id')->find_array();
    ...
    public static function genSMS($invoiceID,$tpl)
    {
        ...
        switch ($tpl){
            case 'created':
                ...
                $tpl = SMSTemplate::where('tpl','Invoice Created')->where('workspace_id',$workspace_id)->first();

                break;

            case 'reminder':

                $tpl = SMSTemplate::where('tpl','Invoice Payment Reminder')->where('workspace_id',$workspace_id)->first();

                break;

            case 'refund':

                $tpl = SMSTemplate::where('tpl','Invoice Refund Confirmation')->where('workspace_id',$workspace_id)->first();

                break;

            case 'overdue':

                $tpl = SMSTemplate::where('tpl','Invoice Overdue Notice')->where('workspace_id',$workspace_id)->first();
            ...
            case 'confirm':
                ...
                $tpl = SMSTemplate::where('tpl','Invoice Payment Confirmation')->where('workspace_id',$workspace_id)->first();
