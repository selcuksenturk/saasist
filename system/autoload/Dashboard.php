...
Class Dashboard{
    ...
    public static function dataLastTwelveMonthsIncExp(){
        ...
        foreach($months as $month){
            ...
            $x = ORM::for_table('sys_transactions')
                ...
                ->where('type','Income')->where_gte('date',$first_day_this_month)->where_lte('date',$last_day_this_month);
        ...
        foreach($months as $month){
            ...
            $x = ORM::for_table('sys_transactions')
                ->where('workspace_id',$workspace_id)
                ->where('type','Expense')->where_gte('date',$first_day_this_month)->where_lte('date',$last_day_this_month);
    ...
    public static function dataIncExpD($select){
        ...
        for ($i = 0; $i <= 31; $i++) {
            ...
            $x = ORM::for_table('sys_transactions')
                ->where('workspace_id',$workspace_id)
                ->where('type','Income')->where('date',$d1);
            if(!$all_data)
            {
                $x->where('aid',$user->id);
            }
            ...
            $x = ORM::for_table('sys_transactions')
                ->where('workspace_id',$workspace_id)
                ->where('type','Expense')->where('date',$d1);
            if(!$all_data)
            {
                $x->where('aid',$user->id);
            }
...
//        }
    ...
    public static function dataIncVsExp($month=''){
        ...
        $mi = ORM::for_table('sys_transactions')
            ->where('workspace_id',$workspace_id)
            ->where('type','Income')->where_gte('date',$first_day_month)->where_lte('date',$mdate)->sum('cr');
        if($mi == ''){
            $mi = '0.00';
        }
        ...
        $me = ORM::for_table('sys_transactions')
            ->where('workspace_id',$workspace_id)
            ->where('type','Expense')->where_gte('date',$first_day_month)->where_lte('date',$mdate)->sum('dr');
        if($me == ''){
            $me = '0.00';
        }
